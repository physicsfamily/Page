...
⑥ 打开` Keil uVision5 `(即安装的MDK5)工程软件，点击工具栏： ` Project` -> `Open Project`，选择工程文件：`基于STM32 OneNET实验\1.OneNET平台显示温湿度实验\WiFi模块程序\USER\WIFI.uvprojx` 并打开。
...
⑪ 将STLink连接到温湿度节点，点击工具栏： ` Project` -> `Open Project`，选择工程文件：`基于STM32 OneNET实验\1.OneNET平台显示温湿度实验\温湿度模块程序\USER\SHT20.uvprojx` 并打开。
   
![打开工程](assets/STM32/39.jpg)

![选择文件](assets/STM32_OneNET/6.jpg)
...
⑮ 下载完成后，将温湿度节点与WIFI节点拼接，并将USB线与任意底座进行重连操作（即：将STLink的USB线从底座上取下，再重新接上）。

⑯ OneNET平台显示实验数据。(脚本位于：`基于STM32 OneNET实验\1.OneNET平台显示温湿度实验\WiFi连接OneNET脚本文件\wifisample.lua`)。具体操作参考[oneNET](https://codelab.stepiot.com/codelabs/oneNet_080/index.html?index=..%2F..index#0)平台应用手册。

![OneNET平台操作](assets/STM32_OneNET/7.png)
...
![代码目录结构](assets/STM32_OneNET/8.jpg) 
