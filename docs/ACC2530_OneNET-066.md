...
`EndDevice.c`->`SampleApp_ProcessEvent()`函数是任务处理函数。在该函数中调用`PIR_Read()`函数读取检测的反馈，当反馈信号发生变化时调用`SendPIRToCoordinator ()`函数向协调器发送数据。
...
```c
    ...
    { 
        ...
        switch ( pkt->clusterId )
        {
            ...
            sprintf((void*)tempbuf,"%d",pkt->cmd.Data[0]);//转成字符串
